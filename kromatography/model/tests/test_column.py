...
class TestColumnType(unittest.TestCase):
    ...
    def test_construction(self):
        ...
        for key, value in COLUMN_TYPE_DATA.iteritems():
            self.assertEqual(getattr(column_type, key), value, msg=key)
...
class TestColumn(unittest.TestCase, UnittestTools):
    ...
    def test_construction(self):
        ...
        for key, value in COLUMN_DATA.iteritems():
            self.assertEqual(getattr(column, key), value, msg=key)

        for key, value in COLUMN_TYPE_DATA.iteritems():
            self.assertEqual(getattr(column.column_type, key), value, msg=key)

        for key, value in RESIN_DATA.iteritems():
            self.assertEqual(getattr(column.resin, key), value, msg=key)
    ...
    def test_volume_calculation(self):
        column_vol = self.column.volume
        actual_col_vol = UnitScalar(157079.63, units=milliliter)

        assert_almost_equal(column_vol, actual_col_vol, 0)
