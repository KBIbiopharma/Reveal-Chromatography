...
def add_data_inspector(plot, move_callback=None, include_overlay=False,
    ...
    inspector = DataInspectorTool(plot, move_callback=move_callback)
    plot.tools.append(inspector)

    # Create and attach the inspector overlay
    if include_overlay:
        overlay = DataInspectorOverlay(plot, inspector=inspector,
...
class DataInspectorOverlay(TextBoxOverlay):
